...
describe('svg inline loader', function () {
  ...
  it('should handle the `iconic-svg` tag', function () {
    var input = '<iconic-svg name="puzzle-piece"/>';
    var expectedOutput = '<svg role="presentation" focusable="false" xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 8 8"><path d="M3 0c-.28 0-.54.1-.72.28C2.1.46 2 .72 2 1c0 .28.18.48.28.72.03.06.03.16.03.28H0v6h2.31c0-.12-.01-.22-.03-.28C2.18 7.48 2 7.28 2 7c0-.28.1-.54.28-.72C2.46 6.1 2.72 6 3 6c.28 0 .54.1.72.28.18.18.28.44.28.72 0 .28-.18.48-.28.72-.03.06-.03.16-.03.28H6V5.69c.12 0 .22.01.28.03.24.1.44.28.72.28.28 0 .54-.1.72-.28C7.9 5.54 8 5.28 8 5c0-.28-.1-.54-.28-.72C7.54 4.1 7.28 4 7 4c-.28 0-.48.18-.72.28-.06.03-.16.03-.28.03V2H3.69c0-.12.01-.22.03-.28.1-.24.28-.44.28-.72 0-.28-.1-.54-.28-.72C3.54.1 3.28 0 3 0z"/></svg>';
    var actualOutput = load(input);
    should.equal(actualOutput, expectedOutput);
  });
  it('should throw an error if the `iconic-svg` tag is missing the name attribute', function () {
    var input = '<iconic-svg/>';
    (function () {
      (load(input));
    }).should.throw(Error);
  });
  it('should throw an error if the `iconic-svg` tag name attribute contains underscores', function () {
    var input = '<iconic-svg name="puzzle_piece"/>';
    (function () {
      (load(input));
    }).should.throw(Error);
  });
  it('should throw an error if the `iconic-svg` tag name attribute contains spaces', function () {
    var input = '<ion-svg name="puzzle piece"/>';
    (function () {
      (load(input));
    }).should.throw(Error);
  });
